...
func (p *plugin) Generate(file *generator.FileDescriptor) {
	for _, fileName := range p.Request.FileToGenerate {
		if fileName == file.GetName() {
			...
			p.types, p.inputs, p.enums, p.gqlModelNames, p.gqlModels, p.subscriptions, p.mutations, p.queries =
				make(map[string]*Type), make(map[string]*Type), make(map[string]*Type), make(map[*Type]string), make(codegen.TypeMap), nil, nil, nil
			p.generate(file)
		}
	}
}
...
func (p *plugin) generate(file *generator.FileDescriptor) {
	for _, svc := range file.GetService() {
		for _, rpc := range svc.GetMethod() {
			...
			if rpc.GetClientStreaming() {
				p.Warn("client streaming not supported with gql") //TODO
			}
	...
	for _, m := range p.mutations {
		p.Schema.P(m.Name, "(req: ", p.gqlModelNames[p.inputs[m.InputType]], "): ", p.gqlModelNames[p.types[m.OutputType]])
	}
	p.Schema.Out()
	p.Schema.P("}\n\n", "extend type Query {")
	p.Schema.In()
	for _, m := range p.queries {
		p.Schema.P(m.Name, "(req: ", p.gqlModelNames[p.inputs[m.InputType]], "): ", p.gqlModelNames[p.types[m.OutputType]])
	}
	p.Schema.Out()
	p.Schema.P("}\n\n", "extend type Subscription {")
	p.Schema.In()
	for _, m := range p.subscriptions {
		p.Schema.P(m.Name, "(req: ", p.gqlModelNames[p.inputs[m.InputType]], "): ", p.gqlModelNames[p.types[m.OutputType]])
	}
	p.Schema.Out()
	p.Schema.P("}\n")
...
func (p *plugin) graphQLType(field *descriptor.FieldDescriptorProto, inputContext bool) string {
	...
	case descriptor.FieldDescriptorProto_TYPE_GROUP:
		p.Error(errors.New("mapping a proto group type to graphql is unimplemented")) //TODO
